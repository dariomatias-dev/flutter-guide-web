...
test.describe("app integration invariants", () => {
  test("assetlinks.json verifies the Android App Links", async ({
    request,
  }) => {
  ...
  test("app-ads.txt is served for AdMob verification", async ({ request }) => {
    ...
    expect(body).toContain(
      "google.com, pub-3105026055105833, DIRECT, f08c47fec0942fa0",
    );
  });
  ...
  for (const category of [
    ...
    test(`/${category}/x tries to open the app`, async ({ page }) => {
      const deepLinkRequest = page.waitForRequest(
        `flutterguide://open.app/${category}/x`,
      );
  ...
  test("/widgets with no slug does not try to open the app", async ({
    page,
  }) => {
